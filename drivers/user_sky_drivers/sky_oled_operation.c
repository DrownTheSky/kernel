...
extern oled_t *gp_oled;
static const unsigned char F8x16[];
static const unsigned char F6x8[][6];

#define OLED_SCL_SET()      gpiod_set_value(gp_oled->gpiod->desc[0], 1)
#define OLED_SCL_RESET()    gpiod_set_value(gp_oled->gpiod->desc[0], 0)
#define OLED_SDA_SET()      gpiod_set_value(gp_oled->gpiod->desc[1], 1)
#define OLED_SDA_RESET()    gpiod_set_value(gp_oled->gpiod->desc[1], 0)
#define OLED_RES_SET()      gpiod_set_value(gp_oled->gpiod->desc[2], 1)
#define OLED_RES_RESET()    gpiod_set_value(gp_oled->gpiod->desc[2], 0)
#define OLED_DC_SET()       gpiod_set_value(gp_oled->gpiod->desc[3], 1)
#define OLED_DC_RESET()     gpiod_set_value(gp_oled->gpiod->desc[3], 0)

/**
  * @brief  向SSD1306写入一个字节
  * @param  data: 写入的数据或者命令
  * @param  cmd: 数据或者命令的标志
  *            @arg OLED_CMD: 命令
  *            @arg OLED_DATA: 数据
  * @retval None
  */
static void OLED_Write_Byte(uint8_t data, cmd_t cmd)
{
    uint8_t i;
	if (cmd) {
        OLED_DC_SET();
...
/**
  * @brief  开启或关闭OLED显示
  * @param  NewState: 开启或关闭的标志
  *            @arg ENABLE: 开启
  *            @arg DISABLE: 关闭
  * @retval None
  */
static void OLED_Display(uint8_t flag)
{
...
static void OLED_Clear(void)  
{
	...
	for (i = 0; i < 8; i++) {  
		...
		for (n = 0; n < OLED_WIDTH; n++) {
            OLED_Write_Byte(0, OLED_DATA);     //更新显示
        }
	}
}
...
/**
  ...
  * @retval None
  */
static void OLED_ShowChar(uint8_t x, uint8_t y, uint8_t size, const char ch)
{
	uint8_t i = 0;
...
/**
  ...
  * @retval None
  */
static void OLED_ShowString(uint8_t x, uint8_t y, uint8_t size, const char *str)
{
	unsigned char j = 0;
	while (str[j] != '\0') {
		OLED_ShowChar(x, y, size, str[j]);
		if(size == 16) {
            x += 8;
        } else {
            x += 6;
...
static struct oled_operation oled_operations = {
    .init = OLED_Init,
